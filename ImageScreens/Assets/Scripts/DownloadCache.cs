...
namespace ImageScreens
{
    ...
    public class DownloadCache
    {
        ...
        public class Result
        {
            ...
            public static Result NewSuccess(Entry entry)
            {
                return new Result
                {
                    Success = true,
            ...
            public static Result NewFail(string URL)
            {
                return new Result
                {
                    Success = false,
        ...
        public void DownloadFile(IListener listener, string url)
        {
            ...
            if (cache.TryGetValue(url, out var cached))
            {
                ...
                listener.DownloadFinished(Result.NewSuccess(cached));
                return;
            }
        ...
        private void SuccessIndex(UnityWebRequest request, Download download)
        {
            ...
            //
            // The index file can only contain paths on the server, because we can't allow
            // the remote server to control which servers the game will access.
            //
            // We intentionally use primitive string concatenation here rather than Uri
            // resolution. Uri resolution would allow an absolute URL in the index (e.g.
            // "https://evil.example/...") to escape the original server and cause the game
            // to make a request to an arbitrary host.
            //
            // Any such "weird" filename is therefore incorporated into the original URL
            // and will fail rather than causing a request to another server.
        ...
        //
        // This also stops all current downloads.
        // Clear is only expected to be called when the game is shutting down or when the player
        // is returning to the main menu. In both cases, all game objects will be destroyed anyway.
        public void Clear()
        {
            StopDownloads();
            ClearCache();
